...
[[nodiscard]] constexpr const char *to_string(OrderMode m) noexcept {
  constexpr const char *kNames[24] = {
      ...
      "SELL_CLOSE_MARKET",
      "BUY_OPEN_LIMIT_BOTH",
      "SELL_OPEN_LIMIT_BOTH",
      "BUY_CLOSE_LIMIT_BOTH",
      "SELL_CLOSE_LIMIT_BOTH",
      "BUY_OPEN_LIMIT_MAKER_BOTH",
      "SELL_OPEN_LIMIT_MAKER_BOTH",
      "BUY_CLOSE_LIMIT_MAKER_BOTH",
      "SELL_CLOSE_LIMIT_MAKER_BOTH",
      "BUY_OPEN_MARKET_BOTH",
      "BUY_CLOSE_MARKET_BOTH",
      "SELL_OPEN_MARKET_BOTH",
      "SELL_CLOSE_MARKET_BOTH",
  };
...
enum class OrderStatus : std::uint8_t {
  ...
  PARTIALLY_FILLED_TRADE_LITE = 8, // 部分成交, 来自 TRADE_LITE / OKX fills 通道 (轻量成交推送, 区别于 ORDER_TRADE_UPDATE)
  UNKNOWN_ORDER_STATUS = 9,
  Count,
};

inline bool is_order_on_line(OrderStatus s) noexcept { return s == OrderStatus::NEW || s == OrderStatus::PARTIALLY_FILLED; }

[[nodiscard]] constexpr const char *to_string(OrderStatus s) noexcept {
  switch (s) {
  case OrderStatus::PLACE_CREATED:
    return "PLACE_CREATED";
  case OrderStatus::CANCEL_CREATED:
    return "CANCEL_CREATED";
  ...
  case OrderStatus::EXPIRED:
    return "EXPIRED";
  case OrderStatus::PARTIALLY_FILLED_TRADE_LITE:
    return "PARTIALLY_FILLED_TRADE_LITE";
  case OrderStatus::UNKNOWN_ORDER_STATUS:
    return "UNKNOWN_ORDER_STATUS";
  case OrderStatus::Count:
    return "?"; // 终结项,非实际状态
  }
  return "?";
}
